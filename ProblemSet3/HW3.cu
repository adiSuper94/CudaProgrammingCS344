...
__global__ void rgb_to_xyY(float *d_r, float *d_g, float *d_b, float *d_x, float *d_y, float *d_log_Y,
...
__global__ void tonemap(float *d_x, float *d_y, float *d_log_Y, float *d_cdf_norm, float *d_r_new,
                        float *d_g_new, float *d_b_new, float min_log_Y, float max_log_Y, float log_Y_range,
                        int num_bins, int num_pixels_y, int num_pixels_x) {
  int ny = num_pixels_y;
  int nx = num_pixels_x;
  int2 image_index_2d =
      make_int2((blockIdx.x * blockDim.x) + threadIdx.x, (blockIdx.y * blockDim.y) + threadIdx.y);
...
void preProcess(float **d_luminance, unsigned int **d_cdf, size_t *numRows, size_t *numCols,
                unsigned int *numberOfBins, const std::string &filename) {
  ...
  *numCols = numCols__;
  ...
  rgb_to_xyY<<<gridSize, blockSize>>>(d_red, d_green, d_blue, d_x__, d_y__, d_logY__, .0001f, numRows__,
                                      numCols__);
...
void postProcess(const std::string &output_file, size_t numRows, size_t numCols, float min_log_Y,
                 float max_log_Y) {
  ...
  tonemap<<<gridSize, blockSize>>>(d_x__, d_y__, d_logY__, d_cdf_normalized, d_red, d_green, d_blue,
                                   min_log_Y, max_log_Y, log_Y_range, numBins, numRows, numCols);
