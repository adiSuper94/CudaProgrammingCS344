...
void preProcess(float **d_luminance, unsigned int **d_cdf, size_t *numRows, size_t *numCols,
                unsigned int *numBins, const std::string &filename);

void postProcess(const std::string &output_file, size_t numRows, size_t numCols, float min_logLum,
                 float max_logLum);

void cleanupGlobalMemory(void);

// Function from student_func.cu
void your_histogram_and_prefixsum(const float *const d_luminance, unsigned int *const d_cdf,
...
int main(int argc, char **argv) {
  ...
  switch (argc) {
    ...
    case 6:
      ...
      break;
    default:
      std::cerr << "Usage: ./HW3 input_file [output_filename] [reference_filename] "
  ...
  your_histogram_and_prefixsum(d_luminance, d_cdf, min_logLum, max_logLum, numRows, numCols, numBins);
  timer.Stop();
